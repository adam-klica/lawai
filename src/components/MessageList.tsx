...
const MessagesList = () => {
  let { messages, isLoadingAnswer } = useMessages()
  if(messages.length === 3){
    let index = messages[2].content.search(/[.!]\s/);
    if (index !== -1) {
      messages[2].content = messages[2].content.substring(index + 2);
    }
  }
  return (
    <div className="max-w-3xl mx-auto pt-8 pb-20 mb-10">
      {messages?.map((message, i) => {
        ...
        return (
          ...
          >
            {!isUser && (
              <img
                src="https://w7.pngwing.com/pngs/960/696/png-transparent-advocate-lawyer-criminal-law-legal-advice-lawyer-emblem-trademark-people.png"
                className="w-15 h-9 rounded-full"
                alt="avatar"
              />
            )}
            <div
